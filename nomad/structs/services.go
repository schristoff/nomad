...
func (sc *ServiceCheck) validateCommon(allowable []string) error {
	// validate the type is allowable in context
	checkType := strings.ToLower(sc.Type)
	if !helper.SliceStringContains(allowable, checkType) {
		s := strings.Join(allowable, ", ")
...
func (sc *ServiceCheck) validateNomad() error {
	...
	if sc.Expose {
		return fmt.Errorf("expose may only be set in Consul services")
	}

	// success_before_passing is consul only
	if sc.SuccessBeforePassing != 0 {
		return fmt.Errorf("success_before_passing may only be set in Consul services")
	}

	// failures_before_critical is consul only
	if sc.FailuresBeforeCritical != 0 {
		return fmt.Errorf("failures_before_critical may only be set in Consul services")
	}
